...
test('album, study and plans expose only the approved core workflows', async () => {
  ...
  assert.match(study, /一轮复习，正在稳稳推进/)
  assert.match(study, /role="progressbar"/)
  assert.match(study, /class="pg-progress-board"/)
  assert.match(study, /学完就从这里登记/)
  assert.match(study, /postgraduate\/progress/)
  assert.match(study, /修正减少/)
  assert.doesNotMatch(study, /管理科目|今日打卡|openConfig|openCheckIn|postgraduate\/checkin/)
...
test('health, cosmetics and ledger remain compact while preserving real records', async () => {
  ...
  assert.match(budget, /POCKET_META/)
...
test('parcel uses active-only locations, named couple ownership and a gift archive; wish keeps explicit archived state', async () => {
  ...
  assert.match(expressView, /本月已取 \$\{thisMonthPickedCount\} 件/)
  assert.match(expressView, /buildExpressArchive\(pickedHistory\.value/)
  assert.match(expressView, /class="modal-dialog archive-dialog"/)
  assert.match(expressView, /class="archive-achievement"/)
  assert.match(expressView, /互相跑腿 \{\{ activeArchiveMutualHelpCount \}\} 次/)
  assert.match(expressView, /取件日 \{\{ activeArchivePickedDayCount \}\} 天/)
  assert.match(expressView, /紧急救援 \{\{ activeArchiveGroup\.urgent \}\} 次/)
  assert.match(expressView, /v-model="selectedArchiveMonthKey"/)
  assert.match(expressView, /直接选择归档月份/)
  assert.match(expressView, /查看更新的归档月份/)
  assert.match(expressView, /查看更早的归档月份/)
  assert.match(expressView, /v-for="delivery in activeArchiveGroup\.items"/)
  assert.match(expressView, /activeArchiveGroup\.mine/)
  assert.match(expressView, /activeArchiveGroup\.partner/)
  assert.doesNotMatch(expressView, /v-for="group in archiveGroups"[^>]*class="archive-month"/)
