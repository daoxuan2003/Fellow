...
test('mood redesign keeps the reference detail navigation contract', async () => {
  ...
  assert.match(dashboard, /class="mood-preview"/)
  assert.match(dashboard, /最近一段对话/)
  assert.match(dashboard, /openTimeline\(latestConversationDate\)/)
  assert.doesNotMatch(dashboard, /<MoodCommentThread/)
  ...
  assert.match(dashboard, /\.slice\(-2\)/, 'home must remain a bounded preview instead of rendering the full conversation')
})
