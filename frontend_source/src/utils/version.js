...
export const FALLBACK_CHANGELOG = [
  {
    ...
    date: '2026-08-27',
    changes: [
      '💰 分仓预算与真实流水正式联动，记账后自动计算每个分仓的已用、剩余和超支',
      '🧾 新增或修改消费时明确选择扣款分仓，并在保存前预览本次记账后的预算状态',
      '💳 欠款还款自动计入还债分仓，新增、修改、删除流水后无需再手工重算预算',
      '🛡️ 历史未归类流水继续保留并明确提示待归类，不猜测、不回填真实财务数据'
    ]
  },
  {
    version: '9.1.2',
  ...
  },
  {
    ...
    date: '2026-08-26',
    changes: [
      '🐛 修复生产环境新增钱包流水返回 503、导致收支无法保存的问题',
      '🔁 无事务环境支持幂等重试，同一笔提交只生成一条流水并只改变一次余额',
      '🛡️ 转账中断可继续完成，明确失败会补偿余额，未完成流水不会出现在列表中',
      '📡 账户与钱包只在写入完整成功后实时同步，重复重试不会重复广播'
    ]
  },
  {
    version: '9.1.0',
