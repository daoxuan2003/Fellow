      ...
      <template v-else-if="overview">
        ...
        <div v-if="activeTab === 'wallet'" id="wallet-panel-wallet" role="tabpanel" aria-labelledby="wallet-tab-wallet" class="tab-panel">
          <section class="safe-panel" :class="{ deficit: scopeSummary?.safeToSpend < 0, 'long-values': hasLongSafeValue(scopeSummary) }">
            ...
            <dl class="safe-breakdown"><div><dt>流动资产</dt><dd class="wallet-number">{{ formatMoney(scopeSummary?.liquidAssets) }}</dd></div><div><dt>剩余还款预留</dt><dd class="wallet-number">-{{ formatMoney(scopeSummary?.debtReserve) }}</dd></div><div><dt>剩余生活与约定</dt><dd class="wallet-number">-{{ formatMoney((scopeSummary?.essentialReserve || 0) + (scopeSummary?.committedReserve || 0)) }}</dd></div></dl>
          </section>
          ...
          <section aria-labelledby="pocket-heading">
            ...
            <div class="pocket-list">
              <article v-for="pocket in scopeSummary?.pockets || []" :key="pocket.key" class="pocket-row" :class="{ overspent: pocket.overspent > 0 }">
                <header><span class="pocket-shape" :class="`tone-${POCKET_META[pocket.key]?.tone}`" aria-hidden="true"></span><strong>{{ POCKET_META[pocket.key]?.label }}</strong><span class="wallet-number">预算 {{ formatMoney(pocket.amount) }}</span></header>
                <div class="pocket-progress" role="progressbar" :aria-label="`${POCKET_META[pocket.key]?.label}预算使用进度`" aria-valuemin="0" aria-valuemax="100" :aria-valuenow="pocket.progress || 0"><span :style="{ transform: `scaleX(${(pocket.progress || 0) / 100})` }"></span></div>
                <footer><span>已用 <b class="wallet-number">{{ formatMoney(pocket.spent) }}</b></span><strong :class="{ expense: pocket.overspent > 0 }">{{ pocket.overspent > 0 ? `超支 ${formatMoney(pocket.overspent)}` : `还剩 ${formatMoney(pocket.remaining)}` }}</strong></footer>
              </article>
            </div>
            <button v-if="scopeSummary?.unassignedCount" class="unassigned-notice" type="button" @click="activeTab = 'transactions'"><span>还有 {{ scopeSummary.unassignedCount }} 笔待归类</span><strong class="wallet-number">{{ formatMoney(scopeSummary.unassignedSpent) }}</strong><small>补上分仓后，预算会立即重算</small></button>
            <p v-else-if="scopeSummary?.confidence !== 'complete'" class="section-note">先填本周期计划和可用账户，钱包才会给出完整结余。</p>
          </section>
        ...
        <div v-else-if="activeTab === 'plan'" id="wallet-panel-plan" role="tabpanel" aria-labelledby="wallet-tab-plan" class="tab-panel">
          ...
          <section class="plan-usage" aria-labelledby="plan-usage-heading">
            <div class="section-heading"><div><h2 id="plan-usage-heading">预算执行到哪了</h2></div><button v-if="scope === currentUserId" class="text-button" type="button" @click="openPlan">调整预算</button></div>
            <dl class="usage-summary"><div><dt>本周期预算</dt><dd class="wallet-number">{{ formatMoney(scopeSummary?.plannedTotal) }}</dd></div><div><dt>已占用预算</dt><dd class="wallet-number">{{ formatMoney(scopeSummary?.spentTotal) }}</dd></div><div><dt>分仓剩余</dt><dd class="wallet-number">{{ formatMoney(scopeSummary?.remainingTotal) }}</dd></div><div><dt>超支总额</dt><dd class="wallet-number" :class="{ expense: scopeSummary?.overspentTotal > 0 }">{{ formatMoney(scopeSummary?.overspentTotal) }}</dd></div></dl>
            <div class="pocket-list compact"><article v-for="pocket in scopeSummary?.pockets || []" :key="pocket.key" class="pocket-row" :class="{ overspent: pocket.overspent > 0 }"><header><span class="pocket-shape" :class="`tone-${POCKET_META[pocket.key]?.tone}`" aria-hidden="true"></span><strong>{{ POCKET_META[pocket.key]?.label }}</strong><span class="wallet-number">{{ pocket.overspent > 0 ? `超支 ${formatMoney(pocket.overspent)}` : `剩 ${formatMoney(pocket.remaining)}` }}</span></header><div class="pocket-progress" role="progressbar" :aria-label="`${POCKET_META[pocket.key]?.label}预算使用进度`" aria-valuemin="0" aria-valuemax="100" :aria-valuenow="pocket.progress || 0"><span :style="{ transform: `scaleX(${(pocket.progress || 0) / 100})` }"></span></div></article></div>
            <button v-if="scopeSummary?.unassignedCount" class="unassigned-notice" type="button" @click="activeTab = 'transactions'"><span>{{ scopeSummary.unassignedCount }} 笔流水还没占用分仓</span><strong class="wallet-number">{{ formatMoney(scopeSummary.unassignedSpent) }}</strong><small>去补上分仓，计划就会自动对齐</small></button>
          </section>
    ...
    <div v-if="sheet" class="sheet-backdrop" @click.self="closeSheet">
      <section ref="sheetDialog" class="sheet" role="dialog" aria-modal="true" :aria-labelledby="`sheet-${sheet}-title`" @keydown="handleSheetKeydown">
        ...
        <form v-else-if="sheet === 'transaction'" class="sheet-body" @submit.prevent="submitTransaction">
          ...
          <template v-else>
            <fieldset v-if="transactionForm.type === 'expense'" class="transaction-pocket-field"><legend>从哪个分仓扣</legend><p>保存后，已用和剩余预算会自动更新。</p><div class="pocket-choice-grid"><button v-for="(meta, key) in POCKET_META" :key="key" type="button" :class="{ active: transactionForm.walletPocketKey === key }" :aria-pressed="transactionForm.walletPocketKey === key" @click="transactionForm.walletPocketKey = key"><i class="pocket-shape" :class="`tone-${meta.tone}`" aria-hidden="true"></i><span>{{ meta.label }}</span></button></div></fieldset>
            <div v-if="transactionPocketPreview" class="pocket-preview" :class="{ overspent: transactionPocketPreview.overspent > 0 }" role="status"><span>保存后 · {{ POCKET_META[transactionForm.walletPocketKey]?.label }}</span><strong class="wallet-number">{{ transactionPocketPreview.overspent > 0 ? `超支 ${formatMoney(transactionPocketPreview.overspent)}` : `还剩 ${formatMoney(transactionPocketPreview.remaining)}` }}</strong></div>
            <label class="field"><span>分类</span><input v-model.trim="transactionForm.category" list="wallet-category-options" maxlength="20" required><datalist id="wallet-category-options"><option v-for="category in transactionCategories" :key="category" :value="category"/></datalist></label><label class="field"><span>{{ transactionForm.type === 'expense' ? '从哪个账户支付（可选）' : '收入到哪个账户（可选）' }}</span><select v-model="transactionForm.accountId"><option value="">只记流水，不调整账户余额</option><option v-for="account in transactionAccountOptions" :key="account._id" :value="account._id">{{ account.name }} · {{ account.type === 'liability' ? '负债' : '资产' }}</option></select></label><p v-if="selectedTransactionAccount?.type === 'liability' && transactionForm.type === 'expense'" class="form-note">这笔会占用所选预算，同时增加该负债账户余额；以后还款会自动占用“还债”分仓。</p>
          </template>
...
const transactionForm = ref({ type: 'expense', amount: '', category: '餐饮', walletPocketKey: 'living', accountId: '', toAccountId: '', date: todayString(), note: '', currency: 'CNY', requestId: makeRequestId('wallet-transaction') })
...
const transactionAccountOptions = computed(() => transactionForm.value.type === 'income' ? ownAssetAccounts.value : ownAccounts.value)
const transactionCategories = computed(() => transactionForm.value.type === 'income' ? incomeDefaults : expenseDefaults)
const ownSummary = computed(() => overview.value?.summaries?.find(row => String(row.ownerId) === currentUserId.value))
const transactionPocketPreview = computed(() => {
  if (transactionForm.value.type !== 'expense'
    || !isDateInPaydayCycle(transactionForm.value.date, currentMonth.value)) return null
  const key = transactionForm.value.walletPocketKey
  const pocket = ownSummary.value?.pockets?.find(row => row.key === key)
  if (!pocket) return null
  const editingAmount = editingTransaction.value?.type === 'expense'
    && editingTransaction.value?.walletPocketKey === key
    && isDateInPaydayCycle(localDateKey(editingTransaction.value.date), currentMonth.value)
    ? Number(editingTransaction.value.amount || 0) : 0
  const spent = Math.max(0, Number(pocket.spent || 0) - editingAmount) + Math.max(0, Number(transactionForm.value.amount || 0))
  const budget = Number(pocket.amount || 0)
  return {
...
watch(() => transactionForm.value.type, type => {
  if (type === 'expense' && !transactionForm.value.walletPocketKey) transactionForm.value.walletPocketKey = 'living'
  if (['income', 'transfer'].includes(type) && selectedTransactionAccount.value?.type === 'liability') transactionForm.value.accountId = ''
  if (type !== 'transfer') transactionForm.value.toAccountId = ''
})
...
function transactionPocketLabel(transaction) { if (transaction.kind === 'debt_payment') return POCKET_META.debt.label; if (transaction.type === 'income') return '不占预算'; if (transaction.type === 'transfer') return '账户内转'; return POCKET_META[transaction.walletPocketKey]?.label || '待归类' }
function transactionFeedback(key, fallback) { const pocket = ownSummary.value?.pockets?.find(row => row.key === key); if (!pocket) return fallback; return pocket.overspent > 0 ? `${fallback}，${POCKET_META[key].label}已超支 ${formatMoney(pocket.overspent)}` : `${fallback}，${POCKET_META[key].label}还剩 ${formatMoney(pocket.remaining)}` }
...
async function runMutation(operation, copy) { submitting.value = true; try { const result = await operation(); closeSheet(true); await loadWallet({ silent: true }); showToast(typeof copy === 'function' ? copy(result) : copy, 'success'); return result } catch (error) { showToast(error.message || '操作失败，请再试一次', 'error'); return null } finally { submitting.value = false } }
...
function openTransaction(t = null) { if (t?.kind === 'debt_payment' || (t && String(t.creatorId) !== currentUserId.value)) return; editingTransaction.value = t; transactionForm.value = t ? { type: t.type, amount: t.amount, category: t.category || '', walletPocketKey: t.walletPocketKey || '', accountId: t.accountId || '', toAccountId: t.toAccountId || '', date: localDateKey(t.date), note: t.note || '', currency: t.currency || 'CNY', requestId: makeRequestId('wallet-transaction-update'), deleteRequestId: makeRequestId('wallet-transaction-delete') } : { type: 'expense', amount: '', category: '餐饮', walletPocketKey: 'living', accountId: '', toAccountId: '', date: todayString(), note: '', currency: 'CNY', requestId: makeRequestId('wallet-transaction') }; sheet.value = 'transaction' }
async function submitTransaction() { const body = { ...transactionForm.value }; if (body.type === 'expense' && selectedTransactionAccount.value?.type === 'liability') body.kind = 'debt_purchase'; const key = body.type === 'expense' ? body.walletPocketKey : null; const fallback = editingTransaction.value ? '流水已经更新，预算已重算' : '流水已经记下，预算已扣除'; await runMutation(() => api(editingTransaction.value ? `/api/wallet/transactions/${editingTransaction.value._id}` : '/api/wallet/transactions', { method: editingTransaction.value ? 'PUT' : 'POST', body: JSON.stringify(body) }), () => key ? transactionFeedback(key, fallback) : fallback) }
async function deleteTransaction() { if (!editingTransaction.value || !window.confirm('确认删除这条流水？账户余额会同步回滚。')) return; await runMutation(() => api(`/api/wallet/transactions/${editingTransaction.value._id}`, { method: 'DELETE', body: JSON.stringify({ requestId: transactionForm.value.deleteRequestId }) }), '流水已经删除') }
...
.safe-panel.long-values .safe-heading { display: grid; grid-template-columns: minmax(0,1fr) auto; }
.safe-panel.long-values .safe-amount.long-number { font-size: clamp(1.5rem,6.5vw,2rem); }
...
.pocket-row { display: grid; gap: var(--fellow-space-2); min-height: var(--fellow-touch-target-min); padding: var(--fellow-space-3) 0; }
.pocket-row+.pocket-row,.account-row+.account-row { border-top: 1px solid color-mix(in srgb,var(--fellow-ink) 24%,transparent); }
.pocket-row header { display: grid; grid-template-columns: auto minmax(0,1fr) auto; align-items: center; gap: var(--fellow-space-2); }.pocket-row header strong { font-size: 1rem; font-weight: 700; }.pocket-row header>span:last-child { max-width: 16ch; overflow-wrap: anywhere; font-size: .875rem; font-weight: 700; text-align: right; }
.pocket-progress { height: var(--fellow-space-2); overflow: hidden; border: 2px solid var(--fellow-ink); border-radius: var(--fellow-radius-pill); background: var(--fellow-white); }.pocket-progress span { display: block; width: 100%; height: 100%; transform-origin: left; background: var(--fellow-blue); transition: transform var(--fellow-motion-standard) var(--fellow-ease-standard); }.pocket-row.overspent .pocket-progress span { background: var(--fellow-pink); }
.pocket-row footer { display: flex; flex-wrap: wrap; justify-content: space-between; gap: var(--fellow-space-2); font-size: .875rem; }.pocket-row footer span { color: var(--fellow-text-secondary); }.pocket-list.compact .pocket-row { padding-block: var(--fellow-space-2); }
.unassigned-notice { display: grid; grid-template-columns: minmax(0,1fr) auto; gap: var(--fellow-space-1) var(--fellow-space-3); width: 100%; min-height: var(--fellow-touch-target-min); margin-top: var(--fellow-space-3); padding: var(--fellow-space-3); border: 2px solid var(--fellow-ink); border-radius: var(--fellow-radius-control); background: var(--fellow-pink); text-align: left; }.unassigned-notice strong { text-align: right; }.unassigned-notice small { grid-column: 1/-1; font-size: .875rem; }
...
.usage-summary { display: grid; grid-template-columns: repeat(2,minmax(0,1fr)); gap: var(--fellow-space-2); margin: 0 0 var(--fellow-space-4); }.usage-summary div { min-width: 0; padding: var(--fellow-space-2); border: 2px solid var(--fellow-ink); border-radius: var(--fellow-radius-control); background: var(--fellow-white); }.usage-summary dt { font-size: .75rem; font-weight: 700; }.usage-summary dd { margin: var(--fellow-space-1) 0 0; overflow-wrap: anywhere; font-size: .875rem; font-weight: 700; }
...
.transaction-pocket-field { display: grid; gap: var(--fellow-space-2); margin: 0; padding: var(--fellow-space-3); border: 2px solid var(--fellow-ink); border-radius: var(--fellow-radius-card); }.transaction-pocket-field legend { padding: 0 var(--fellow-space-2); font-size: .875rem; font-weight: 700; }.transaction-pocket-field>p { margin: 0; color: var(--fellow-text-secondary); font-size: .875rem; }.pocket-choice-grid { display: grid; grid-template-columns: repeat(2,minmax(0,1fr)); gap: var(--fellow-space-2); }.pocket-choice-grid button { display: flex; align-items: center; gap: var(--fellow-space-2); min-height: var(--fellow-touch-target-min); padding: var(--fellow-space-2); border: 2px solid var(--fellow-ink); border-radius: var(--fellow-radius-control); background: var(--fellow-white); font-size: .875rem; font-weight: 700; text-align: left; }.pocket-choice-grid button.active { background: var(--fellow-mint); }.pocket-choice-grid button:last-child:nth-child(odd) { grid-column: 1/-1; }
.pocket-preview { display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: var(--fellow-space-2); padding: var(--fellow-space-2) var(--fellow-space-3); border: 2px solid var(--fellow-ink); border-radius: var(--fellow-radius-control); background: var(--fellow-blue); font-size: .875rem; }.pocket-preview.overspent { background: var(--fellow-pink); }
...
@media (max-width:340px) { .safe-amount { font-size: 2rem; }.safe-breakdown { grid-template-columns: 1fr; }.income-forecast>span { display: grid; gap: var(--fellow-space-1); }.income-forecast strong { max-width: 100%; text-align: left; }.next-payment-row { grid-template-columns: auto minmax(0,1fr); }.due-action { grid-column: 1/-1; grid-template-columns: minmax(0,1fr) auto; align-items: center; justify-items: start; }.field-pair { grid-template-columns: 1fr; }.pocket-row header { grid-template-columns: auto minmax(0,1fr); }.pocket-row header>span:last-child { grid-column: 2; max-width: none; text-align: left; } }
@media (prefers-reduced-motion:reduce) { .loading-mark { animation: none; }.progress-track span,.debt-progress span,.pocket-progress span,.toast-enter-active,.toast-leave-active,.sync-enter-active,.sync-leave-active { transition: none; } }
</style>
